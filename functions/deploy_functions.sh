...
DATASET_ID="freshflow_dataset"    # Replace with your BigQuery dataset ID
...
# Base directory - now running from functions folder, so use current directory
BASE_DIR="."
...
gcloud functions list --filter="name:freshflow-*" --format="table(name,status,trigger.httpsTrigger.url)"
